...
## 8. Bônus: Governança de IA local com Okam CLI

O **Okam** é o "caderno de memórias e regras" do seu assistente de IA local. Com ele, você estabelece ganchos de automação (hooks) e skills (pacotes de instruções e regras) para garantir que a IA atue sob restrições estritas de conformidade e segurança.

### 1. Inicializando o Okam no seu projeto
Navegue até a raiz do seu projeto e execute:
```bash
okam init
```
Esse comando cria o arquivo de configuração `.okamrc` no diretório do projeto. A partir deste arquivo, você poderá definir hooks, skills ativas e configurações específicas.

### 2. Comandos Básicos de Sessão
- **Listar sessões ativas**: `okam sessions`
- **Executar comando único rápido**: `okam run "pergunta de código"`
- **Encerrar a sessão ativa**: `okam exit`

### 3. Automação de Governança com Hooks (Ganchos)
Os hooks no Okam permitem automatizar tarefas críticas antes (`pre-command`/`pre-session`) e depois (`post-command`/`post-session`) de interagir com o agente.

Exemplo de configuração no `.okamrc` para impedir regressões ou falhas:
```yaml
hooks:
  pre-command:
    # Atualiza as ramificações locais antes de propor modificações
    - "git fetch origin"
  post-command:
    # Roda o validador de sintaxe ou testes automaticamente para checar a IA
    - "npm run lint"
```

### 4. Modo Pipe para Auditoria (Revisão de Diffs)
Você pode integrar o Okam com o Git usando pipes do terminal, ideal para validar códigos antes de efetuar commits e commits de segurança:
```bash
# Revisar o diff em staging para encontrar segredos vazados ou bugs arquiteturais
git diff --cached | okam run "revise este diff e aponte problemas de segurança, credenciais expostas ou más práticas"
```

### 5. Estendendo a Inteligência com Skills
As skills são pacotes de diretrizes que instruem o assistente a agir sob determinados padrões técnicos ou de negócios (como a skill `checklists` para ADHD ou `clean-code`).
- **Ver skills ativas**: `okam skill list`
- **Instalar uma skill oficial**: `okam skill install checklists`
- **Atualizar todas as skills**: `okam skill update --all`
